...
user_dir_army(piece_info_t *obj, loc_t loc)
{
	...
	fatal (obj, loc,
	       "Sorry, Commander. There is no more room on the transport.  Do you insist? ",
	...
	if (!getyn ( /* thanks to Craig Hansen for this next message */
		"Troops can't walk on water, Commander.  Do you really want to go to sea? "))
	    return;
	...
	if (!getyn (
		"Commander, those are our troops!  Do you really want to attack them? "))
	    return;
...
user_dir_fighter(piece_info_t *obj, loc_t loc)
{
	...
	fatal (obj, loc,
	       "That's never worked before, commander.  Do you really want to try? ",
	       "Your fighter was shot down.");

    else if (isupper (game.user_map[loc].contents)) {
	if (!getyn ("Commander, those are our troops!  "
...
user_dir_ship(piece_info_t *obj, loc_t loc)
{
	...
	if (!getyn ("Ships need sea to float, commander.  Do you really want to go ashore? ")) return;
...
move_army_to_city(piece_info_t *obj, loc_t city_loc)
{
    ...
    if (tt != NULL) move_obj (obj, city_loc);
		...
		"That's our city, Commander!  Do you really want to attack the garrison? ",
